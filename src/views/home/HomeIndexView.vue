                    ...
                    <div class="card-body">
                      ...
                      <div style="text-align: center;">
                        ...
                        <div class="row" style="text-align: center;">
                          <div class="col-7">
                            <button v-if="signInData.isSignInToday == 0" @click="signIn()" class="btn btn-sign-in">签到</button>
                            <button v-if="signInData.isSignInToday == 1" @click="signIn()" class="btn btn-sign-in disabled">已签到</button>
                          </div>
                          <div class="col-5">
                            <p>
                        ...
                        <div class="collapse" id="collapseExample">
                          <div class="card card-body">
                            第一天获得1硬币，第二天获得2硬币...第七天获得7硬币，之后每天均获得7硬币。
                          </div>
                        </div>
                    </div>
                </div>
...
import {userSigninService, userSigninResultService, userSigninContinueDaysService, userisSigninTodayService} from '@/api/signin.js'
...
export default {
    created() {
      ...
      // 签到数据
      this.signInData.continueDays = userSigninContinueDaysService();
      this.signInData.res = userSigninResultService().data
      this.signInData.isSignInToday = userisSigninTodayService().data
    },
    data() {
      return {
        pageNum: 1,
    ...
    methods:{
        ...
        // 签到
        signIn(){
          let result = userSigninService()
          if(result.code === 1){
              commonUtil.message(result.message, "danger");
          }
          if(result.code === 0){
              commonUtil.message(result.message + ',获得' + result.data + '枚硬币！', "success");
              this.now_user = userInfoByIdService(this.now_user.id).data;
              this.signInData.continueDays = userSigninContinueDaysService();
              const userInfoStore = useUserInfoStore()
              this.now_user = userInfoByIdService(this.now_user.id).data
              userInfoStore.setInfo(this.now_user)
              this.signInData.res = userSigninResultService().data
              this.signInData.isSignInToday = userisSigninTodayService().data
          }
        },
